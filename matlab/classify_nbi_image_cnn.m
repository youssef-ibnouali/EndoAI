...
function [result_img_path, scores] = classify_nbi_image_cnn(image_path, output_dir)
    patch_size = 200;
    step = 5;
    iou_thresh = 0.01;
    ...
    parfor i = 1:total
        ...
        temp_patch_path = fullfile(tempdir, sprintf('patch_%d.png', i));
        imwrite(patch_rgb, temp_patch_path);

        [status, cmdout] = system(sprintf('python matlab/predict_patch.py "%s"', temp_patch_path));
        label = strtrim(cmdout);

        delete(temp_patch_path);
