...
%    • Apply classification **rules derived from a trained decision tree**:
%         - The tree uses thresholds on features to classify into:
%           ▸ 'Normal'
...
cancer_count = 0;
normal_count = 0;
% load('kmeans_model.mat', 'C', 'cluster_labels'); 
...
parfor i = 1:total_patches
    ...
    % === Classification rules based on decision tree ===
    if saturation < 0.217
        if edge_density < 0.0222
            label = 'Dysplasia'; color = 'r'; dysplasia_count = dysplasia_count + 1;
        else
            label = 'Cancer'; color = 'm'; cancer_count = cancer_count + 1;
        end
    
    elseif lbp.std < 382.296
        if saturation < 0.4301
            label = 'Normal'; color = 'g'; normal_count = normal_count + 1;
        else
            label = 'IM'; color = 'y'; im_count = im_count + 1;
        end
    
    elseif fractal < 1.6583
        if wavelet.cH < 77915.9
            label = 'Normal'; color = 'g'; normal_count = normal_count + 1;
        else
            if saturation < 0.34705
                label = 'IM'; color = 'y'; im_count = im_count + 1;
            else
                if glcm.Contrast < 51.1752
                    label = 'IM'; color = 'y'; im_count = im_count + 1;
                else
        ...
        if glcm.Correlation < 0.7999
            label = 'Normal'; color = 'g'; normal_count = normal_count + 1;
        else
            if wavelet.cH < 287080
                label = 'AG'; color = 'c'; ag_count = ag_count + 1;
            else
                label = 'IM'; color = 'y'; im_count = im_count + 1;
            end
